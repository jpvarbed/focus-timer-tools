...
3. CI (`.github/workflows/post-merge-smoke.yml`) runs both on every push to main, after the
   hosted-MCP Vercel deploy that a merge triggers. Push-only (never `pull_request`) so this public
   repo's `FOCUS_API_KEY` secret is never exposed to fork PRs. The secret is a **dedicated
   `ci-smoke` minted key**, not a personal one — independently revocable in focus Settings without
   disrupting a human.
